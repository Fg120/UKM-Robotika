...
import BidangEdit from './bidangEdit';
import BidangDelete from './bidangDelete';
...
export default function BidangIndex({ bidangs, filters }: Props) {
  ...
  const handleDelete = (bidang: Bidang) => {
    setSelectedBidang(bidang);
      ...
      <Card>
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold">Manajemen Bidang</h1>
          ...
          <div className="mb-6">
            <div className="flex gap-2">
              <Input
                placeholder="Cari bidang berdasarkan nama atau deskripsi..."
                value={search}
          ...
          {bidangs.meta && bidangs.meta.last_page > 1 && (
            <div className="flex justify-center mt-6">
              <div className="flex gap-2">
                {bidangs.links.map((link, index) => (
                  <button
                    key={index}
          ...
          <BidangCreate isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} />
          {selectedBidang && (
            <BidangEdit
              isOpen={isEditOpen}
              onClose={() => { setIsEditOpen(false); setSelectedBidang(null); }}
              bidangId={selectedBidang.id}
            />
          )}
          {selectedBidang && (
            <BidangDelete
              isOpen={isDeleteOpen}
              onClose={() => { setIsDeleteOpen(false); setSelectedBidang(null); }}
              bidangId={selectedBidang.id}
            />
          )}
        </div>
