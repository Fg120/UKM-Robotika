...
export default function PengurusEdit({ isOpen, onClose, pengurusId }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await fetch(`/admin/pengurus/${pengurusId}/edit`); const json = await res.json();
        ...
        {loading ? (<div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin" /></div>) : data ? (
          <form onSubmit={handleSubmit} className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-4 items-center gap-3">
              ...
              <div className="col-span-3">
                <Select value={form.sub_divisi_id} onValueChange={(v)=> setForm({ ...form, sub_divisi_id:v })}>
