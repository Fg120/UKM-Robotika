...
export default function TagEdit({ isOpen, onClose, tagId }: TagEditProps) {
  ...
  const loadTag = async () => {
    try {
      const response = await fetch(`/admin/tags/${tagId}/edit`);
      const data = await response.json();
      setTag(data.tag);
      setFormData({ nama: data.tag.nama || '', deskripsi: data.tag.deskripsi || '' });
    } catch (error) {
      // noop
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await router.put(`/admin/tags/${tagId}`, formData, {
        onSuccess: () => {
          setTag(null);
          onClose();
        },
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Ubah detail tag sesuai kebutuhan.
          </DialogDescription>
        </DialogHeader>
